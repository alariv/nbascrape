...
export async function getLeagueStats(url) {
  ...
  return new Promise((resolve) => {
    // console.log('getLeagueStats start')

    Object.keys(years).map((year, idx) => {
      // console.log('url', url.replace('{{year}}', years[idx]));
      request(
        {
          method: 'GET',
        ...
        (err, response, body) => {
          counter += 1;
          // console.log(
          //   'finished getLeagueStats request from',
          //   url.replace('{{year}}', years[idx])
          // );
          //   console.error(err)
          ...
          $(rows).each(function (i, row) {
            ...
            //   console.log($(row).find('td[data-stat=team]').text())
